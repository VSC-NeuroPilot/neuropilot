...
export function patienceDiff(aLines: string[], bLines: string[], diffPlusFlag = false): PatienceDiff {
    ...
    function findUnique(arr: string[], lo: number, hi: number): Map<string, number> {
        const lineMap = new Map<string, number | IndexCount>();

        for (let i = lo; i <= hi; i++) {
            const line = arr[i];

            if (lineMap.has(line)) {
                (lineMap.get(line)! as IndexCount).count++;
                (lineMap.get(line)! as IndexCount).index = i;
            } else {
                lineMap.set(line, {
                    count: 1,
                    index: i,
        ...
        lineMap.forEach((val, key, map) => {
            if ((val as IndexCount).count !== 1) {
                map.delete(key);
            } else {
                map.set(key, (val as IndexCount).index);
            }
        });

        return lineMap as Map<string, number>;
    }
    ...
    function uniqueCommon(aArray: string[], aLo: number, aHi: number, bArray: string[], bLo: number, bHi: number): Map<string, LinkedIndexPair> {
        ...
        return ma as Map<string, LinkedIndexPair>;
    }
    ...
    function longestCommonSubsequence(abMap: Map<string, LinkedIndexPair>): LinkedIndexPair[] {
        const ja: LinkedIndexPair[][] = [];
    ...
    function addToResult(aIndex: number, bIndex: number): void {
        if (bIndex < 0) {
    ...
    function addSubMatch(aLo: number, aHi: number, bLo: number, bHi: number): void {
...
export function patienceDiffPlus(aLines: string[], bLines: string[]): PatienceDiff {
