...
export const REQUEST_COOKIE_ACTION: RCEAction = {
    ...
    promptGenerator: (context: RCEContext) =>
        `have a${context.data.params?.flavor ? ' ' + context.data.params.flavor : ''} cookie.`,
    ...
    switch (permission) {
        case PermissionLevel.COPILOT: {
            ...
            return actionHandlerSuccess(
                `Waiting on ${CONNECTION.userName} to decide on the flavor.`,
                'Waiting for cookie flavor...',
            );
        }
        case PermissionLevel.AUTOPILOT: {
            ...
            if (actionData.params?.flavor) {
                ...
                return actionHandlerSuccess(
                    `You grabbed a ${actionData.params.flavor} cookie!`,
                    `${actionData.params.flavor} cookie grabbed`,
                );
            }
            ...
            return actionHandlerFailure(
                base + quotes[randomIndex],
                `${base.replace('You', CONNECTION.nameOfAPI)}${quotes[randomIndex].replace(/you|You/g, CONNECTION.nameOfAPI)} (undefined flavor)`,
            );
        }
        default:
            return actionHandlerFailure(
                'Uhhhh, something went wrong',
                "Somehow, this action's permission wasn't set to Copilot or Autopilot, huh???",
            ); // Added for type-safety I guess
    }
...
export function giveCookie(
    ...
    updateStatus?: SimplifiedStatusUpdateHandler,
) {
    if (!NEURO.connected) {
        ...
        .then(flavor => {
            if (!flavor) {
                ...
                if (isRequested) {
                    ...
                    updateStatus?.(
                        'failure',
                        `${quotes[randomIndex].replace('your', `${CONNECTION.nameOfAPI}'s`)} (undefined flavor)`,
                    );
                }
                return;
            }
